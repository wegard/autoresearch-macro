...
Note on paper integration:
  The emitted tab_calibration_macros.tex is \\input{}-ed from
  paper/tables/auto_macros.tex so that paper/main.tex can cite the
  calibration macros (\\pooledFTeighty, \\pooledCaleightyGap, etc.) alongside
  the existing point-forecast macros generated by
  src/tables/generate_tables.py. Keep both macro files regeneratable
  independently; the paper preamble only does a single
...
def write_macros(
    ...
    # Note: LaTeX control-sequence names must be all letters, so we
    # use "eighty" / "fifty" suffixes rather than numeric "80" / "50".
    lines.append(_cmd("pooledFTeighty", pooled["ft80"]))
    lines.append(_cmd("pooledZSeighty", pooled["zs80"]))
    lines.append(_cmd("pooledFTfifty", pooled["ft50"]))
    lines.append(_cmd("pooledZSfifty", pooled["zs50"]))
    lines.append(_pct("pooledFTeightyGap", pooled["ft80"], 0.8))
    lines.append(_pct("pooledZSeightyGap", pooled["zs80"], 0.8))
    lines.append(_pct("pooledFTfiftyGap", pooled["ft50"], 0.5))
    lines.append(_pct("pooledZSfiftyGap", pooled["zs50"], 0.5))
    ...
    lines.append(_cmd("pooledCaleighty", pooled.get("cal80")))
    lines.append(_cmd("pooledCalfifty", pooled.get("cal50")))
    lines.append(_pct("pooledCaleightyGap", pooled.get("cal80"), 0.8))
    lines.append(_pct("pooledCalfiftyGap", pooled.get("cal50"), 0.5))
