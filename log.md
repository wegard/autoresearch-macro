...
3. **Sweden's quick→full eval gating has a structural bias.** The baseline scores 1.0274 on the 20-origin quick eval but 1.0056 on the full 120-origin eval — a 2.1% gap, full is better. The search uses `quick_score < state.best_score` to decide whether to spend a full eval, where `state.best_score` is the current best *full* score. So a candidate has to score below 1.0056 on quick eval to even be evaluated properly — about 2% below the actual quick-eval baseline. In the second informed run and the blind run, **0 of 50** proposals cleared this bar. The two-phase pre-filter is dropping legitimate Sweden candidates en masse.
   ...
   **Fixed 2026-04-11.** Audit revealed the gap exists for all three countries (Norway -3.63%, Canada -0.67%, Sweden +2.17%) but only Sweden's direction destroys improvements — Norway and Canada have *quick better than full*, so the old gate was just permissive (wasted compute, no improvements lost). Sweden's *full better than quick* direction made the gate too strict.

   **Fix:** Added `best_quick_score` to `SearchState`, updated alongside `best_score` whenever a config is accepted on full eval. Gating now uses `quick_score < state.best_quick_score` instead of `quick_score < state.best_score`. The gate is now direction-agnostic — it asks "is this candidate's quick score better than the quick score of the best config seen so far?", which is well-defined regardless of how quick eval relates to full eval. The greedy search has the same fix. Old state files are migrated by recovering the quick score from the accepted history record matching `best_score`. Three new regression tests cover the new field, the legacy fallback, and the history-based recovery. Norway and Canada existing results don't need re-running because their old gating was permissive, not destructive — every accepted improvement is real. **Sweden informed and blind LLM searches should be re-run** to recover the improvements that the old gating discarded.
