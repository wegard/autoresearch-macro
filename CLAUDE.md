...
- `src/train.py` — agent sandbox: loads Chronos-2 via AutoGluon, applies covariate selection and fine-tuning config. **The search agent proposes changes to the config section at the top.**
...
- Use `chronos-forecasting` and `autogluon.timeseries` as the model interface
